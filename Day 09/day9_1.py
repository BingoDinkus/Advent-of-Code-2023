...
for line in f:
    ...
    for i in range(len(seq) - 2, -1, -1):
        curr_row_last = seq[i][-1]
        next_row_last = seq[i  + 1][-1]
        new_value = curr_row_last + next_row_last
        log.debug(f'[{line_ct:3}] Row {i}, {curr_row_last} + {next_row_last} = {new_value}')
        seq[i].append(new_value)

    # Take the final element from the first row and add it to our final list
    final_list.append(seq[0][-1])
